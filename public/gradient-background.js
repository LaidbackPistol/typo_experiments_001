...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function initShaders() {
    ...
    // Updated fragment shader with properly centered mirror effect
    const fsSource = `
    ...
    float snoise(vec3 v) {
      // [existing noise function code remains the same]
      // ...
    }

    // Get color from gradient
    vec3 getColor(float value) {
      // [existing getColor function code remains the same]
      // ...
    }
    ...
    void main() {
      // Start with texture coordinates (0-1 range)
      ...
      if (aspect > 1.0) {
        aspectCorrected.x = uv.x * aspect - (aspect - 1.0) * 0.5; // Center horizontally
      } else {
        aspectCorrected.y = uv.y / aspect + (1.0 - 1.0/aspect) * 0.5; // Center vertically
      }
      
      // Center coordinates at origin (0,0 is now the center of screen)
      vec2 centered = aspectCorrected - 0.5;
      
      // Apply scale (centered scaling)
      vec2 scaled = centered / uScale;
      
      // Mirror if needed (symmetry at origin)
      if (uMirrorMode == 1 || uMirrorMode == 3) { // X mirror or both
        scaled.x = abs(scaled.x);
      }
      if (uMirrorMode == 2 || uMirrorMode == 3) { // Y mirror or both 
        scaled.y = abs(scaled.y);
      }
      ...
      vec3 noisePos = vec3(
        translated.x + uTime * uAnimationSpeed,
        translated.y + uTime * (uAnimationSpeed * 0.5),
        uSeed / 1000.0
      );
      ...
      float displacement = paperTexture(translated);
      color += displacement;
